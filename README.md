...
* 요구사항 설치
  * 요구사항 설치 중 오류 - ERROR: ERROR: Failed to build installable wheels for some pyproject.toml based projects (pyworld)
    ...
    2. Visual C++ Build Tools 설치 시, "C++ CMake Tools for Windows"나 "MSVC v142 - VS 2019 C++ x64/x86 build tools"와 같은 항목을 선택하여 설치
* 데이터 전처리 - dataset기반 멜 스펙토로그램 생성
  1. 데이터를 학습할 수 있는 형식으로 변환하는 과정
  2. 명령어
  ```
...
* 모델 추론
  ...
  2. 명령어
  ```
  python synthesize.py --text "안녕하세요" --speaker_id 0
  ```
